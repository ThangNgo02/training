...
class ResponseCode {
  ...
  static find(response: IDataResponse, state: State) {
    try {
      LoggerService.debug(
        "ResponseCode execute find receive response",
        response,
      );
      ...
      if (Helper.isEmpty(funcName)) {
        throw new Error(
          `ResponseCode code ${response.code} with funcName not defined`,
        );
      }
      const stateFunc: any = Helper.isEmpty(state) ? "" : state[funcName];
      if (Helper.isEmpty(stateFunc)) {
        throw new Error(
          `ResponseCode code ${response.code} key of funcRequest not defined`,
        );
      }
      stateFunc(response.data);
    } catch (error: any) {
      LoggerService.error(
        "ResponseCode execute find receive error",
        error.toString(),
      );
      throw error;
    }
