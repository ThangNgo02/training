...
export type PutFormValues = FixedShiftFormValues | FlexibleShiftFormValues;
...
const PutForm = ({
  ...
  isOpen: boolean;
  totalData: any[];
  shiftData?: Partial<PutFormValues>;
  setStatus: (status: 'activate' | 'deactivate') => void;
}) => {
  ...
  useEffect(() => {
    if (isOpen && shiftData) {
      ...
      // For FIXED type
      if (shiftData.shiftType === 'FIXED') {
        const fixedData = shiftData as Partial<FixedShiftFormValues>;
        formRef.current?.reset({
          code: fixedData.code ?? '',
          name: fixedData.name ?? '',
          note: fixedData.note ?? '',
          departmentList: fixedData.departmentList ?? [],
          shiftType: 'FIXED',
          status: fixedData?.status ?? 'ACTIVE',
          overtime: fixedData.overtime ?? false,
          overtimeForMinutes: fixedData.overtimeForMinutes ?? 30,
          workingTimes: fixedData.workingTimes ?? defaultFormValues.workingTimes,
        });

        setIsOvertimeEnabled(fixedData.overtime ?? false);
        setOvertimeMinutes(fixedData.overtimeForMinutes ?? 30);

        // Update times state if workingTimes exist
        if (fixedData.workingTimes) {
          const newTimes = { ...times };
          for (const [index, workingTime] of fixedData.workingTimes.entries()) {
            const dayKey = weekdays[index].key;
            newTimes[dayKey] = {
              enabled: workingTime.active,
              startTime: workingTime.startTime,
              endTime: workingTime.endTime,
              fromBreakTime: workingTime.fromBreakTime,
              toBreakTime: workingTime.toBreakTime,
              totalHours: workingTime.totalHours,
              isTimeError: false,
              isBreakTimeError: false,
            };
          }
        ...
        // For FLEXIBLE type
        const flexibleData = shiftData as Partial<FlexibleShiftFormValues>;
        formRef.current?.reset({
          code: flexibleData.code ?? '',
          name: flexibleData.name ?? '',
          note: flexibleData.note ?? '',
          departmentList: flexibleData.departmentList ?? [],
          shiftType: 'FLEXIBLE',
          startTimeForShift: flexibleData.startTimeForShift ?? '00:00',
          endTimeForShift: flexibleData.endTimeForShift ?? '00:00',
          minimumTimeForShift: flexibleData.minimumTimeForShift ?? 1,
        });

        setMinimumTimeForShift(flexibleData.minimumTimeForShift ?? 1);
        setMinimumTimeDisplay(formatDecimal(flexibleData.minimumTimeForShift ?? 1));
      }

      setShiftType(shiftData.shiftType ?? 'FIXED');
  ...
  const handleChangeStatus = () => {
    ...
    const newStatus = shiftData.status === 'ACTIVE' ? 'deactivate' : 'activate';
    setStatus(newStatus);
  };
          ...
          <div className='h-screen w-screen overflow-y-auto bg-white '>
            <div className='h-full overflow-y-auto'>
              <Form
                ...
                defaultValues={shiftData ?? defaultFormValues}>
                <div
                  onClick={e => {
                    e.stopPropagation();
                  }}
                  ...
                  <div className='flex justify-between'>
                    <h1 className='text-3xl font-bold'>Chỉnh sửa ca làm việc</h1>
                      ...
                      <div className='space-y-4'>
                        <div>
                          ...
                          <InputRoot
                            ...
                            className='w-full rounded-md border px-3 py-2'
                            disabled={shiftData?.status === 'DEACTIVE'}
                            errorString={
                              // eslint-disable-next-line @typescript-eslint/no-base-to-string
                ...
                <div className='mb-3 h-20 w-full'>
                  <div className='flex justify-between'>
                    <div>
                      <h1>{shiftData?.status === 'ACTIVE' ? 'Khóa ca' : 'Mở khóa ca'}</h1>
                      <div className='mt-3'>
                        {shiftData?.status === 'ACTIVE'
                          ? 'Khóa tất cả thông tin ca làm việc này.'
                          : 'Mở khóa tất cả thông tin ca làm việc này.'}
                      </div>
                    </div>
                    <Button
                      className={`mt-6 hover:bg-gray-500 ${
                        shiftData?.status === 'ACTIVE'
                          ? 'border-red-500 bg-white text-red-500  '
                          : 'border-green-500 bg-white text-green-500 '
                      }`}
                      onClick={handleChangeStatus}>
                      {shiftData?.status === 'ACTIVE' ? 'Khóa' : 'Mở khóa'}
                    </Button>
                  </div>
                </div>
              </Form>
